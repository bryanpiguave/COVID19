...
mean_values = [0.3 25 650 0.15E6 5.13E-2 3.65E-2 940 3.5]; % four variable
...
GSA.dso = 2.5E6;% FIXED! Densidad del solido (Kg/m3)
...
for sample = 1:np
    ...
    GSA.D2 = M1(sample,6);
    GSA.Ts = M1(sample,7);
    GSA.Lfin = M1(sample,8);
    
    Y(sample) = inputRCU_new(GSA);
...
for sample=1:np %sample size
    ...
    GSA.D2 = M2(sample,6);
    GSA.Ts = M2(sample,7);
    GSA.Lfin = M2(sample,8);
    
    YR(sample) = inputRCU_new(GSA);
...
for i=1:nd %variables or parameters in sensitivity analysis
    ...
    for sample = 1:np % number of samples
        ...
        GSA.D2 = NT(sample,6);
        GSA.Ts = NT(sample,7);
        GSA.Lfin = NT(sample,8);
        
        YTp(sample,i) = inputRCU_new(GSA);
