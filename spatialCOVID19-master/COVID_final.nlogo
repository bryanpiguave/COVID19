...
[
 ...
 ;;;;; Para regresion
  inf1 dist1 vo1 gro real mse_inf
  cur dist2 vo2 gro2 real2 mse_cur
  mue dist3 vo3 gro3 real3 mse_mue
  data_i data_m data_c
 dias_test
  ff
  data_i2
data_m2
...
[
  ...
  vulnerable?               ;; está en un grupo de riesgo ;percent of people in a risk group (senior, diabetes, heart condition, etc.)
  detected?            ;; diagnosed
  asintomatico?        ;;; If true the person is asintomatica
  v                    ;;; variable para escoger vulnerables que sobreviviran al uci
...
to setup
  ...
  ; Para extraer data de un .csv (la data debe estar columna 1 infectados columna 2 muertos)

;  let data csv:from-file "example.csv"
;  let data_ext matrix:from-row-list data
;  let data_write matrix:to-column-list data_ext
;  set data_c last data_write
;  set data_i first data_write
;  let aux butfirst data_write
;  set data_m first aux
;  let dimension matrix:dimensions data_ext

;  set dias_test first dimension

  set gro n-values 1[0]
  set gro2 n-values 1[0]
  set gro3 n-values 1[0]
  set data_i2 n-values 1 [0]
  set data_m2 n-values 1 [0]
  set data_c2 n-values 1 [0]
...
to setup-globals
  ask patches [set pcolor white set hospital? false set uci? false]
  set hospitales? true
  set ucis? true
  ...
  if hospitales? [
    ...
    ask patches with [pxcor >= 0 and pxcor < capacidad-hospital and pycor = 0 ]
    [ set pcolor blue set hospital? true]
    set p-tendencia-hospitalizacion 50
  ]

   if ucis? [
    set capacidad-uci camas-uci
    ask patches with [pxcor >= 0 and pxcor < capacidad-uci and pycor = 1 ]
    [ set pcolor gray set uci? true]
    set p-tendencia-hospitalizacion 50
  ]
...
;;;; 30% de la poblacion es asintomatica
   ask n-of (round poblacion * 0.3) turtles
  [ set susceptible? true
    set infection-length random recovery-time
    set detected? false
    set asintomatico? true]
;.......................................................
...
to assign-tendency ;; Turtle procedure
  ...
  if asintomatico? = true [set tendencia-hospitalizacion 0 ]                     ;;;; personas asintomaticas no tienen tendencia a hospitalizacion
    if (random-float 100 < Vulnerables) [ set vulnerable? true]                  ;;;;; asignacion de vulnerables
...
to assign-color ;; turtle procedure
  ...
  if asintomatico? = true [set shape "arrow" ] ;  identificar asintomaticos
end
...
;    [ stop ]
  ...
  ask turtles
    [ if not isolated? and not hospitalized?
      ...
      and detected?
        [ isolate ] ]
;.......PROCEDIMIENTO PARA ASEGURAR DISPONIBILIDAD DE CAMAS
  if hospitales? [
    ask turtles
    [
      if not hospitalized?
...
tick
  ;regresion regresion2 regresion3
 ; fitness
  if ticks = 120 [stop]            ;;;; detener simulacion en dias_test
  ;if ticks = 28 [ask turtle 1 [set infected? true ]]
end

to calcular-salud

  if infected? and not asintomatico? [ ; excluye a los asintomaticos de perder la salud
    ifelse hospitalized?[
      ifelse vulnerable?
      [set salud salud - 0.1]    ;riesgo, hospitalizado vulnerable
      [set salud salud - 0.05]   ; riesgo, hospitalizado no vulnerable
    ]
    [ifelse vulnerable?
      [set salud salud - 0.5]    ;riesgo, no hospitalizado vulnerable
      [set salud salud - 0.02]   ;riesgo, no hospitalizado no vulnerable
    ]
   ]
  ...
  if salud <= 0 [
   set muertes (muertes  + 1)
    ...
    if hospitalized? = true [
      ifelse vulnerable? = true
      [set hospitalized? false
    set capacidad-uci capacidad-uci + 1
      ]
        [set hospitalized? false
...
;;;;;;; condicion de muerte para el 50% de pacientes en uci, que haya superado los 15 dias de infeccion

    if v = 1 and infection-length >= 15 [
      set muertes (muertes  + 1)
    ask (patch-at 0 0)
    [
...
;;;; procedimiento para evitar que las turtles se muevan sobre hospitales y uci

  while [detect-obstacle? = true]
  [rt random-normal 0 90 fd random-normal movilidad movilidad / 2 move-to patch-here]
end
to-report detect-obstacle?

  ifelse  [pcolor] of patch-here = blue
  or [pcolor] of patch-here = gray + 3
  or [pcolor] of patch-here = blue + 3
  or [pcolor] of patch-here = gray
  [report true]
  [report false]
end
...
to isolate ;; turtle procedure
  ...
  ask (patch-at 0 0) [ set pcolor gray + 2]
end
...
to hospitalize ;; turtle procedure
 ...
 if isolated? [
  ...
  set capacidad-uci capacidad-uci - 1
    let empty-uci patches with [ uci? = true and pcolor = gray ]
    if any? empty-uci [ move-to one-of empty-uci
    ask patch-here [set pcolor gray   + 3] set v random 2]

  ]
 [
       set hospitalized? true
  set capacidad-hospital capacidad-hospital - 1
    let empty-hosp patches with [ hospital? = true and pcolor = blue ]
    if any? empty-hosp [ move-to one-of empty-hosp]
    ...
    [
      ...
      [
        ...
        [
          set infected? true
          if random-float 100 < por-deteccion [set detected? true]
          if asintomatico? = true [set detected? false]
...
to regresion ; infectados

  set inf1  (count turtles with [ infected?])
  set gro lput inf1 gro
  set data_i2 lput (item (ticks - 1) data_i) data_i2

  set real map [ i -> (i) ] data_i2
  let resta map [ i -> abs (i * i) ] (map - (butfirst gro)  (butfirst real) )
  let inv (1 /(2 * (length butfirst gro)))
  let suma reduce + resta
  set mse_inf (inv * suma)


end

to regresion2 ; curados


  set cur  (count turtles with [ cured?])
  set gro2 lput cur gro2
  set data_c2 lput (item (ticks - 1) data_c) data_c2

  set real2 map [ i -> (i) ] data_c2
  let resta map [ i -> abs (i * i) ] (map - (butfirst gro2)  (butfirst real2) )
  let inv (1 /(2 * (length butfirst gro2)))
  let suma reduce + resta
  set mse_cur (inv * suma)



end

to regresion3 ; muertos


  set gro3 lput muertes gro3
  set data_m2 lput (item (ticks - 1) data_m) data_m2

  set real3 map [ i -> (i) ] data_m2
  let resta map [ i -> abs (i * i) ] (map - (butfirst gro3)  (butfirst real3) )
  let inv (1 /(2 * (length butfirst gro3)))
  let suma reduce + resta
  set mse_mue (inv * suma)
...
to fitness ; cost function

  let mse_mue_t 0.5
  let mse_cur_t 0.5
  let mse_inf_t 0.5

  set ff ((((mse_mue - mse_mue_t) / mse_mue_t) ^ 2)  + (((mse_inf - mse_inf_t) / mse_inf_t) ^ 2) + (((mse_cur - mse_cur_t) / mse_cur_t) ^ 2))  / 3
...
"Muertos" 1.0 0 -2674135 true "" "plot muertes"
"Curados" 1.0 0 -13840069 true "" "plot (count turtles with [ cured?])"
...
"" ""
PENS
"Capacidad Hospitalaria" 1.0 0 -16777216 true "" "plot camas-uci + camas"
...
574
Asintomaticos
count turtles with [asintomatico? = true]
17
1
11
